...
def test_from_kwargs_preserves_resolved_child_config():
    """Recursive callers can reuse a child config without losing options."""
    child = GhostConfig.from_kwargs(
        {"navigate": True, "nav_instruction": "x", "wait_until": "load"}
    ).for_child()

    assert GhostConfig.from_kwargs(child) is child


def test_recursive_read_html_uses_for_child_config(monkeypatch):
    """Recursive children receive the root config with navigation disabled."""
    seen_ghost_opts = []

    def fake_read_html_content(url, ghost_opts=None):
        seen_ghost_opts.append(ghost_opts)
        if url == "https://example.com":
            return {
                "raw": "[Child](https://example.com/child)",
                "segments": [],
                "title": "Root",
                "url": url,
            }
        return {"raw": "", "segments": [], "title": "Child", "url": url}

    monkeypatch.setattr(utils, "read_html_content", fake_read_html_content)
    utils.recursive_read_html(
        "https://example.com",
        depth=2,
        ghost_opts={"navigate": True, "nav_instruction": "open results"},
    )

    assert len(seen_ghost_opts) == 2
    child_config = seen_ghost_opts[1]
    assert isinstance(child_config, GhostConfig)
    assert child_config.navigate is False
    assert child_config.nav_instruction is None
